...
def safe_nearest_frames_choice(input_data, size, frames):
    ...
    rand_ind = np.random.choice(np.arange(size//2, len(input_data) - size//2))
    if size % 2 == 0:
        if np.random.binomial(n=1, p= 0.5):     # more down
            return_data = input_data[rand_ind - size//2:rand_ind + size//2]
        else:
            return_data = input_data[rand_ind - size//2 + 1:rand_ind + size//2 + 1]
...
class MPerClassSampler_weighted(Sampler):
    """
    ...
    def __init__(self, labels, frames, m, batch_size=None, length_before_new_iter=100000):
    ...
    def __iter__(self):
        ...
        for _ in range(num_iters):
            ...
            for label in curr_label_set:
                ...
                frames = np.array(self.frames)[t]
                idx_list[i : i + self.m_per_class] = safe_nearest_frames_choice(
                    t, size=self.m_per_class, frames=frames
                )
                i += self.m_per_class
